...
function env_check(&$env_items) {
	...
	$env_items[] = array('name' => 'PHP版本', 'min' => '5.6', 'good' => '5.6', 'cur' => PHP_VERSION, 'status'=>(PHP_VERSION < 5.6 ? 0:1));
	$tmp = function_exists('gd_info') ? gd_info() : array();
	preg_match("/[\d.]+/", $tmp['GD Version'],$match);
	unset($tmp);
	$env_items[] = array('name' => 'GD库', 'min' => '2.0', 'good' => '2.0', 'cur' => $match[0], 'status' => ($match[0] < 2 ? 0:1));
	$env_items[] = array('name' => 'ZIP库', 'min' => '支持', 'good' => '支持','cur' => in_array('zip', get_loaded_extensions())?'支持':'不支持', 'status' => in_array('zip', get_loaded_extensions())?1:0);
