...
<![endif]--> 
  ...
  <?php include('../admin_config/admin_list.php');?>
		...
		<?php
		$ad_top_json_url=YYCMS_ROOT."ad/ad_float/ad.json";
		...
		$bankinfo = array();
		 $date = $txt::select($_GET['id']);
			...
			if($ad_top_id =="1544970714"){$str_err_name="底部广告";}
...
$ad_top_md=$ad_top_md.'--'.$ad_top_url_zt;	
	if ($ad_top_url_zt ==null) { echo'<script language="javascript">alert("状态不可为空"); </script>';exit();}
	if ($ad_top_url ==null) { echo'<script language="javascript">alert("广告链接URL不可为空"); </script>';exit();}
	if ($ad_top_pic ==null) { echo'<script language="javascript">alert("广告图片素材地址URL不可为空"); </script>';exit();}
...
<!-- 
layer.msg('恭喜修改成功');
window.location.href="ad_float.php" 
